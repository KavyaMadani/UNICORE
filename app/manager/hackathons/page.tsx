...
import {
  ...
  School, Tag, ChevronRight, Clock, TrendingUp,
} from 'lucide-react';
import { useRouter } from 'next/navigation';
import { formatDate } from '@/lib/utils';

const STATUS_BG: Record<string, { bg: string; border: string; glow: string }> = {
  active:   { bg: 'rgba(16,185,129,0.05)',  border: 'rgba(16,185,129,0.18)',  glow: 'rgba(16,185,129,0.12)'  },
  upcoming: { bg: 'rgba(99,102,241,0.05)',  border: 'rgba(99,102,241,0.18)',  glow: 'rgba(99,102,241,0.12)'  },
  ended:    { bg: 'rgba(255,255,255,0.02)', border: 'rgba(255,255,255,0.07)', glow: 'transparent'             },
  draft:    { bg: 'rgba(251,191,36,0.04)',  border: 'rgba(251,191,36,0.15)',  glow: 'rgba(251,191,36,0.08)'  },
};
...
export default function ManagerHackathonsPage() {
  ...
  const filtered = MOCK_HACKATHONS.filter(h => filter === 'all' || h.status === filter);
  const counts = {
    all: MOCK_HACKATHONS.length,
    active: MOCK_HACKATHONS.filter(h => h.status === 'active').length,
    upcoming: MOCK_HACKATHONS.filter(h => h.status === 'upcoming').length,
    ended: MOCK_HACKATHONS.filter(h => h.status === 'ended').length,
    draft: MOCK_HACKATHONS.filter(h => h.status === 'draft').length,
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 36 }}>
        {[
          { label: 'Total Events', value: MOCK_HACKATHONS.length, icon: <Zap size={18} color="#818cf8" />, color: '#818cf8' },
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
            <div style={{
              ...
              <div style={{ fontSize: 30, fontWeight: 800, color: '#f1f5f9', lineHeight: 1, marginBottom: 6 }}>{s.value}</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 28, flexWrap: 'wrap' }}>
        {(['all', 'active', 'upcoming', 'ended', 'draft'] as const).map(f => (
          ...
          >
            {f} {filter === f && <span style={{ opacity: 0.7 }}>({counts[f]})</span>}
          </button>
        ))}
      </div>

      {/* Hackathon cards - list layout for spaciousness */}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 24, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 260 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10, flexWrap: 'wrap' }}>
                        <h3 style={{ fontSize: 21, fontWeight: 800, color: '#f1f5f9', lineHeight: 1.2 }}>{hack.title}</h3>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 13, color: '#64748b' }}>{hack.organizer}</span>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap', marginBottom: 28, paddingBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                        {[
                          { icon: <Users size={14} color="#60a5fa" />, value: hack.participantCount.toLocaleString(), label: 'Participants' },
                          { icon: <Trophy size={14} color="#fbbf24" />, value: hack.prizePool, label: 'Prize Pool' },
                          { icon: <Calendar size={14} color="#34d399" />, value: formatDate(hack.startDate), label: 'Start Date' },
                          { icon: <Calendar size={14} color="#f87171" />, value: formatDate(hack.endDate), label: 'End Date' },
                        ].map(stat => (
                      ...
                      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                        <button
                          ...
                          onClick={() => router.push(`/manager/hackathons/${hack.id}`)}
                        ...
                        <button
                          onClick={() => router.push(`/manager/hackathons/${hack.id}/edit`)}
                          style={{
                            display: 'flex', alignItems: 'center', gap: 7,
                            padding: '10px 20px', borderRadius: 12,
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 0', color: '#475569' }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>No {filter} hackathons</p>
          <p style={{ fontSize: 14, color: '#334155', marginBottom: 24 }}>Create one to get started.</p>
          <Button onClick={() => router.push('/manager/hackathons/create')} leftIcon={<Plus size={14} />}>
            Create Hackathon
          </Button>
        </div>
      )}
